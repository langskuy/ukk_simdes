...
<body>
    ...
    <div class="header-section">
        ...
        @else
            <div style="width:55px; height:55px; background:#f0f8f0; border:2px solid #228b22; border-radius:4px; display:flex; align-items:center; justify-content:center; color:#228b22; font-weight:bold; font-size:18px;">{{ strtoupper(substr($village['nama_desa'] ?? 'D', 0, 1)) }}</div>
        @endif
    ...
    <table class="data-table">
        ...
        <tr>
            <td>Tahun Berdiri</td>
            <td>:</td>
            <td>{{ $kObj['tahun_berdiri'] ?? '—' }}</td>
        </tr>
        <tr>
            <td>Skala Produksi</td>
            <td>:</td>
            <td>{{ $kObj['skala_produksi'] ?? '—' }}</td>
        </tr>
    </table>

    <!-- Body Surat -->
    <div class="letter-body">
        <p>Dengan ini kami menerangkan bahwa orang yang tersebut di atas adalah benar warga penduduk Desa {{ $village['nama_desa'] ?? 'Wonokasian' }} yang memiliki dan menjalankan usaha di desa kami.</p>
        
        <p>Surat keterangan usaha ini diberikan berdasarkan verifikasi lapangan dan data yang tercatat di administrasi desa untuk keperluan administrasi, perizinan, atau keperluan lainnya yang sah.</p>
